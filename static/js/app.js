...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // loadLanguages(); // 已注释：HTML中已有完整的语言选项列表
...
function initializeEventListeners() {
    ...
    const reportDownloadModalElement = document.getElementById('reportDownloadModal');
    const reportDownloadModal = reportDownloadModalElement ? new bootstrap.Modal(reportDownloadModalElement) : null;
    ...
    document.querySelectorAll('.report-format-option').forEach((button) => {
        button.addEventListener('click', async () => {
            if (reportDownloadModal) {
                reportDownloadModal.hide();
            }
            await downloadReport(button.dataset.format || 'pdf');
        });
    });
...
async function downloadReport(format = 'json') {
    ...
    try {
        ...
        const extension = extensionMap[normalizedFormat] || 'json';

        showLog('info', `[*] 正在下载 ${extension.toUpperCase()} 报告...`);
        ...
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', `migration_report_${currentMigrationId.slice(0, 8)}.${extension}`);
        document.body.appendChild(link);
        link.click();
        link.remove();
        window.URL.revokeObjectURL(url);

        showLog('success', `[OK] ${extension.toUpperCase()} 报告下载完成`);
        showAlert('成功', `[OK] ${extension.toUpperCase()} 报告下载完成`, 'success');
    } catch (error) {
